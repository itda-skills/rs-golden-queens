...
# KIS 호출 전체 wall-clock 예산(초). flow-kr 잡 타임아웃(5분) 안에서 retry 누적이
# 부분실패 노출(아래 경고)을 막지 않도록 KIS 섹션 전체에 deadline 을 건다(#10 I8).
# 정상 수집(~40s 안팎)의 여유 배수이며, 초과 시 남은 KIS 콜을 즉시 중단해 발송 보장.
_KIS_BUDGET_SEC = 120
...
def _collect_kis_sections(client, data: dict, warnings: list[str]) -> None:
    ...
    """
    # KIS 섹션 전체에 wall-clock 예산을 걸어, 장애 시 retry 누적이 잡 타임아웃(5분)을
    # 넘겨 발송 자체를 막는 일이 없게 한다(#10 I8). 초과분은 빈 결과 → 아래 경고.
    client.set_budget(_KIS_BUDGET_SEC)
    _t0 = time.monotonic()
    ...
    # 예산 대비 실제 소요를 남겨 운영 로그에서 _KIS_BUDGET_SEC 적정성을 추적한다(#10 I8).
    print(f"⏱️  KIS 섹션 소요 {time.monotonic() - _t0:.1f}s / 예산 {_KIS_BUDGET_SEC}s")
