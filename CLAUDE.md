...
GitHub Actions가 정해진 시각에 트리거하며, 수집 데이터는 저장하지 않고 즉시 발송한다.
...
nas/                   # NAS 컨테이너 실행 스크립트
tests/                 # pytest (unit/integration/live)
main.py, scheduler.py  # 로컬·NAS 스케줄 실행 진입점
schedule.toml          # 외부화된 cron 스케줄
Dockerfile, entrypoint.sh
.github/workflows/     # GitHub Actions (flow-kr / flow-us / flow-weekly)
```

진입점·스케줄 관련 세부는 `README.md`, `market_flow/README.md`, `schedule.toml` 참조.

## 도메인 규약

- **사실 데이터만** — 투자 권유·종목 추천·매수/매도 시점 판단을 출력하지 않는다.
- **저장 금지** — 수집 데이터는 디스크에 영구 저장하지 않고 텔레그램으로만 전송.
- **휴장 처리** — KR/US 비거래일에는 "[KR/US] 오늘은 휴장입니다" 한 줄 발송. 캘린더 판단은 `calendar_utils.py` 경유.
- **DST 자동 반영** — 미국장 시각은 EDT/EST 전환을 코드가 처리한다. 하드코딩 금지.
- **발송 단일 경로** — Telegram 전송은 반드시 `telegram_push.py` 를 통한다. `MARKET_FLOW_DRY_RUN=1` 환경변수는 stdout 출력만 하고 실제 발송하지 않는다 (테스트·디버깅에 활용).
...
7. **드라이브이로직 자제** — 요청 범위 밖의 리팩토링·정리는 별도 제안으로 분리한다.
...
- GitHub Actions cron 은 제거됨. NAS 컨테이너가 `schedule.toml` 기준으로 실행한다.
